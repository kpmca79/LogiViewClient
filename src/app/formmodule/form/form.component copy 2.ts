...
import { FrmResponse, ResponseField, UploadFile } from 'app/model/Respons';
...
export class FormComponent2 implements OnInit {
    ...
    surl = "https://localhost:4200/form/600010b4b19f802d0cd61649";
    ...
    public overrideMeterialCSS() {
        ...
        $(document).ready(
            function changeMatCSS() {
                ...
                if (tr == 0 && tg == 0 && tb == 0) { tr = 150; tg = 150; tb = 150; }
    ...
    saveForm(showNotification: boolean) {
        ...
        const saveFormObj = this.frmSrv.saveForm(this.frm, this.formID)
            .subscribe(response => {
                ...
                if (showNotification)
                    this.showNotification('top', 'center', 'Form saved successfully', 's', 2000);
                this.addFieldValidations();
            });
    ...
    dataURLtoFile(dataurl): File {
        ...
        const imageFile: File = new File([u8arr], fileName, {
            type: "png"
        });
        return imageFile;
    }
    async uploadSignature(field: FormField, f: File): Promise<any> {
        if (!f) return;
        ...
        let promise = new Promise((resolve) => {
            this.frmSrv.uploadResource(f).subscribe(
                data => {
                    ...
                    let upFile: UploadFile = new UploadFile();
                    upFile.id = result.fileid;
                    upFile.type = f.type;
                    upFile.size = f.size;
    ...
    async getPublicIP(): Promise<any> {
        return; // keyur change this to redece time in getting ip
        let promise = new Promise((resolve) => {

            const ipObservable = this.frmSrv.getPublicIP().subscribe(data => {
                let result: any = data;
                this.respJson.IpAddress = result.ip;
                console.log("IIIIIIIIIIIIPPPPPPPPPPPPPPPPPPPPPP-->", result.ip);
    ...
    setDropDownVal(field: FormField) {
        ...
        if (field.dropDownMultiLevel && field.dropDownMultiSelect) {
            ...
            if (value) {
                ...
                if (multiOption) {
                    multiOption.forEach((val, key) => {
                        let opAr = val.split(",");
                        let subF: ResponseField = new ResponseField();
                        subF.displayName = key;
                        subF.listVal = opAr;
                        subF.valueType = "list"
            ...
            if (value && value.includes('@opt@')) {
                ...
                subF.displayName = keyval[0];
                subF.value = keyval[1];
                subF.valueType = "string"
    ...
    async completePayment(frm: Form): Promise<any> {
        ...
        let promise = new Promise((resolve) => {
            if (field.type == 'productlist' && field.productList && field.productList.total > 0) {
                ...
                }, error => { console.log("999999"); console.log("Payment processing error ", error); resolve(''); }
                )
            }
            else{
                console.log("No productlist found");
                resolve('');
            }
        });
        return promise;
        ...
        if (!isError) {
            ...
            for (field of this.formField) {
                ...
                if (field.type != "section" && field.type != "submit") {
                    ...
                    if (field.type == 'signature') {
                    ...
                    else if (field.type == "fullname" || field.type == "address") {
                        ...
                        if (field.subfields) {
                            let selTime: String = '';
                            selTime = field.subfields[0].message //hour
                            selTime = selTime + ":" + field.subfields[1].message //min
                            selTime = selTime + " " + field.subfields[2].message //am/pm
                        ...
                        if (field.checked)
                            this.respJson[field.id] = true;
                        else
                            this.respJson[field.id] = false;

                    }
                    else if (field.type == 'number' || field.type == 'numberspinner') {
                        this.respJson[field.id] = { type: "number", value: value };
                    }
                    //else if ( field.fname )
                    else if (value) {
                        console.log(" Oter field=", field);
                        console.log("Other fileds name=", field.name, " value=", value);
                        this.respJson[field.id] = value;
    ...
    public getFullName(field: FormField): string {
        let fullname: string = "";
        if (field.subfields) {
            if (field.subfields[0] && field.subfields[0].visible) fullname = field.subfields[0].frmControl.value + " ";
            if (field.subfields[1] && field.subfields[1].visible) fullname = fullname + field.subfields[1].frmControl.value + " ";
            if (field.subfields[2] && field.subfields[2].visible) fullname = fullname + field.subfields[2].frmControl.value + " ";
            if (field.subfields[3] && field.subfields[3].visible) fullname = fullname + field.subfields[3].frmControl.value;
        }
    ...
    saveFormResponse() {
        ...
        const formObj = this.frmSrv.saveResponse(this.formID, this.respJson).subscribe(
            ...
            error => {
                console.log(error);
                this.saveResponseLoader = false;
                this.showNotification('top', 'center', 'Opps something went wroing while registering your informatoin. Please try again later.', 'f', 4000)
                return;
            });
    }
    ...
    doPayment(paymentDetail: PaymentDetail): Observable<any> {
        return new Observable(observer => {
            this.frmSrv.getPaymentHash(paymentDetail).subscribe(data => {
                ...
                console.log("Generated hash by tomcat server for payment="+data.data.txnid);
            ...
            }, error => { console.log("333333"); console.log(error) });
